...
from src.models.prompts import VERIFIER_METHOD_PROMPT, VERIFIER_PROMPT, VERIFIER_SYSTEM
...
class ResponseVerifierAgent(BaseAgent):
    ...
    def __init__(self, llm_client: LLMClient):
        super().__init__(
            name="ResponseVerifier", llm_client=llm_client, system_prompt=VERIFIER_SYSTEM
        )
    ...
    def verify(
        ...
        if symbolic_result is not None:
            self._log_action(
                "symbolic_verification", {"success": True, "result": symbolic_result.is_correct}
            )
    ...
    def verify_method(
        ...
        try:
            ...
            m = methods[0]
            steps_text = "\n".join(
                f"{s.get('step', '')}: {s.get('action', '')}" for s in m.get("steps", [])
            )
    ...
    def _verify_symbolic(
        self, correct_answer: str, student_response: str
        ...
        try:
            ...
            # parse_expr вместо eval: студенческий ввод не должен исполняться как Python
            try:
                correct_val = float(
                    parse_expr(
                        correct_clean.replace("^", "**"), transformations=self.transformations
                    )
                )
                student_val = float(
                    parse_expr(
                        student_clean.replace("^", "**"), transformations=self.transformations
                    )
                )
                ...
                pass  # не число — пробуем символьное сравнение ниже
    ...
    def _verify_with_llm(self, task: Task, student_response: str) -> VerificationResult:
        """Verify answer using LLM."""

        prompt = VERIFIER_PROMPT.format(
            problem=task.problem, correct_answer=task.answer, student_response=student_response
        )
        ...
        try:
            ...
            return VerificationResult(
                is_correct=False, confidence=0.3, feedback="Could not verify answer automatically."
            )
