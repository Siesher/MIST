...
class Settings(BaseSettings):
    ...
    MODEL_NAME: str = Field(
        ...
        description="Primary model (GSPO fine-tuned Qwen3.5-9B, Socratic tutor)",
    )
    ...
    # ─────────────────────────────────────────────────────────────
    INITIAL_MASTERY: float = Field(default=0.3, description="Initial skill mastery level")
    ...
    COGNITIVE_OVERLOAD_THRESHOLD: float = Field(
        default=0.75, description="Score above which cognitive overload is detected"
    )
    ...
    SESSION_MEMORY_MAX_TURNS: int = Field(
        default=20, description="Maximum conversation turns to keep in session memory"
    )
    ...
    API_TOKEN_EXPIRE_HOURS: int = Field(default=24, description="JWT token expiration in hours")

    # ─────────────────────────────────────────────────────────────
    # Extended Features: Gamification
    # ─────────────────────────────────────────────────────────────
    GAMIFICATION_ENABLED: bool = Field(default=True, description="Enable gamification features")
    XP_MULTIPLIER: float = Field(default=1.0, description="XP multiplier for events")
    STREAK_FREEZE_COUNT: int = Field(default=3, description="Number of streak freezes available")
    ...
    AFFECTIVE_WINDOW_SIZE: int = Field(
        default=5, ge=1, description="Number of recent messages to analyze for affective state"
    )
    AFFECT_DETECTOR_TYPE: str = Field(
        default="rules", description="Affect detector type: 'rules' (rule-based) or 'ml' (RuBERT)"
    )
    AFFECT_ML_MODEL_PATH: str = Field(
        default="data/models/rubert_affect", description="Path to fine-tuned RuBERT affect model"
    )
    ...
    VISION_ANALYZER_ENABLED: bool = Field(default=True, description="Enable vision-based math OCR")
    ...
    COT_MIN_DIFFICULTY: str = Field(
        default="medium", description="Minimum difficulty level to use CoT (easy/medium/hard)"
    )
