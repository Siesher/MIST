...
class CodeExecutor:
    ...
    def execute(
        self, code: str, input_data: str = "", timeout: Optional[float] = None
    ) -> ExecutionResult:
        """
        ...
        if not is_safe:
            return ExecutionResult(status=ExecutionStatus.SECURITY_ERROR, error=error)
        ...
        try:
            ...
            return ExecutionResult(status=ExecutionStatus.RUNTIME_ERROR, error=str(e))
        finally:
    ...
    def run_tests(
        self, code: str, test_cases: List[TestCase], stop_on_first_fail: bool = False
    ) -> SubmissionResult:
        """
...
class ErrorAnalyzer:
    ...
    def analyze_wrong_answer(
        cls, actual: str, expected: str, test_input: str = ""
    ) -> Dict[str, Any]:
        ...
        if not actual_lines:
            ...
            return {"type": "Нет вывода", "hints": hints, "diff": None}

        # Разная длина вывода
        if len(actual_lines) != len(expected_lines):
            hints.append(f"Количество строк: {len(actual_lines)}, ожидалось: {len(expected_lines)}")
