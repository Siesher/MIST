...
class TutoringSession(BaseModel):
    ...
    def add_student_message(self, content: str) -> None:
        ...
        self.conversation.append(ConversationTurn(role="student", content=content))
        self.attempts += 1

    def add_tutor_response(self, response: TutorResponse) -> None:
        """Add a tutor response to conversation."""
        self.conversation.append(
            ConversationTurn(role="tutor", content=response.message, move=response.move)
        )
...
class StudentProfile(BaseModel):
    ...
    def get_weak_skills(self, threshold: float = 0.5) -> List[str]:
        ...
        return [skill for skill, mastery in self.skill_mastery.items() if mastery < threshold]
...
# ═══════════════════════════════════════════════════════════════════════════
# ToM-Tutor (feature 017): Student belief state inferred by MentalModelAgent
# ═══════════════════════════════════════════════════════════════════════════


class BeliefState(BaseModel):
    """Structured snapshot of what the student is believed to think right now.

    Produced by MentalModelAgent at the MENTAL_MODEL pipeline stage and
    consumed by Planner (strategy selection) and Navigator (gap re-ranking).

    Ephemeral — one instance per tutoring turn, not persisted. Logged to
    session audit log under "mental_model" key.
    """

    active_misconception: Optional[str] = None
    """Short RU description of most likely active misconception. None if unclear."""

    active_misconception_node_id: Optional[str] = None
    """Knowledge Forge node ID if misconception matches an existing graph node.
    Filled by the system (graph lookup), not by the LLM."""

    belief_about_topic: str = ""
    """One RU sentence: how the student seems to model the current topic."""

    predicted_reactions: Dict[str, str] = Field(default_factory=dict)
    """Map of teaching strategy name -> predicted student reaction (RU).
    Keys match TeachingStrategy values (e.g. 'scaffolded', 'conceptual_repair')."""

    candidate_misconceptions: List[str] = Field(default_factory=list)
    """Up to 3 other plausible misconceptions ranked by likelihood."""

    confidence: float = Field(default=0.0, ge=0.0, le=1.0)
    """0.0-1.0. ToM inference reliability self-assessed by the model.
    Downstream consumers MUST ignore other fields when confidence < 0.3."""

    reasoning: str = ""
    """Brief LLM explanation of the inference. For audit logging only."""

    generated_at: str = Field(default_factory=lambda: datetime.now().isoformat())
    """ISO 8601 timestamp of inference."""

    profile_used: str = ""
    """Active resource profile under which this inference ran ('lite'/'standard'/'max')."""

    @classmethod
    def empty(cls, profile_name: str = "") -> "BeliefState":
        """Create an empty BeliefState with confidence=0.

        Returned by MentalModelAgent when LLM fails, times out, or is disabled.
        Downstream consumers check confidence == 0 and fall back to baseline.
        """
        return cls(
    ...
    def is_usable(self, min_confidence: float = 0.3) -> bool:
        """Check if the belief state is trustworthy enough to use."""
