Ядро системы MITS я реализовал как мультиагентный пайплайн по паттерну GenMentor [9]: каждый агент инкапсулирует одну педагогическую функцию (профилирование, планирование, реплика, верификация) и взаимодействует с остальными через стандартизированные интерфейсы. Такая декомпозиция давала мне три практических преимущества: независимое тестирование агентов в изоляции, возможность замены отдельных компонентов без переписывания соседних и устойчивость системы при сбоях (graceful degradation).
