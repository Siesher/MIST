...
def check_format(response: str) -> tuple[bool, bool, bool]:
    has_steps = any(marker in response.lower() for marker in [
        "шаг", "step", "следовательно", "значит", "поэтому",
        "1.", "2.", "3.", "подставим", "найдём", "вычислим",
    ])
...
def main():
    ...
    for domain, questions in TESTS.items():
        ...
        for question, expected in questions:
            ...
            try:
                ...
                flags = ("S" if has_steps else ".") + ("L" if has_latex else ".") + ("T" if has_thinking else ".")
    ...
    print(f"Step-by-step reasoning: {sum(r.has_steps for r in results)}/{len(results)} = {sum(r.has_steps for r in results)/len(results)*100:.0f}%")
    print(f"LaTeX formatting:       {sum(r.has_latex for r in results)}/{len(results)} = {sum(r.has_latex for r in results)/len(results)*100:.0f}%")
    print(f"Chain-of-thought tags:  {sum(r.has_thinking for r in results)}/{len(results)} = {sum(r.has_thinking for r in results)/len(results)*100:.0f}%")
    ...
    with open("evaluation/reports/quick_eval_gspo.json", "w", encoding="utf-8") as f:
        json.dump(out, f, indent=2, ensure_ascii=False)
    print(f"\nSaved to evaluation/reports/quick_eval_gspo.json")
